...
class CaseStmtFuncGenerator
  ...
  def self.matcher_function_from_file(path, func_name, env_var_names = DEFAULT_ENV_VAR_NAMES)
    new(env_var_names).matcher_function_from_file(path, func_name)
  end
  ...
  def matcher_function_from_file(path, func_name)
    lines = File.readlines(path, chomp: true)
    function_lines = generate_function(lines, func_name)
    clean_up_output(function_lines)
  end
